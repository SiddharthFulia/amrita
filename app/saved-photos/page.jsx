...
function Lightbox({ photos, index, onClose, onPrev, onNext, onDownload, downloading }) {
    ...
    >
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <button onClick={() => onDownload(photo)} style={{
            background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.25)',
            borderRadius: '50px', padding: '7px 18px', color: '#fff',
            fontSize: '13px', fontWeight: 600, cursor: downloading === photo.id ? 'wait' : 'pointer',
            fontFamily: "'Inter', sans-serif",
          }}>{downloading === photo.id ? '⏳' : '↓ Download'}</button>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: '13px', fontFamily: "'Inter', sans-serif", margin: '0 0 28px' }}>
          {count} photo{count !== 1 ? 's' : ''} will be moved to deleted pics.
        </p>
...
export default function SavedPhotosPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/photobooth-saves', { cache: 'no-store' });
  ...
  const doDelete = async (ids) => {
    ...
    try {
      const res = await fetch('/api/photobooth-saves', {
        method: 'DELETE',
      ...
      if (lightboxIndex !== null) {
        ...
        if (currentPhoto && idSet.has(currentPhoto.id)) {
  ...
  const deleteSingle = (photo) => {
    setConfirmTarget({ ids: [photo.id], count: 1 });
  };

  const downloadPhoto = async (photo) => {
    setDownloading(photo.id);
    try {
      const res = await fetch(`/api/download?url=${encodeURIComponent(photo.fullUrl)}`);
    ...
    <div style={{ minHeight: '100vh', background: '#07071a', padding: '40px 16px 80px', maxWidth: '960px', margin: '0 auto' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '32px' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.4)', fontFamily: "'Inter', sans-serif", fontSize: '14px', margin: '0 0 16px' }}>
          {photos.length > 0 ? `${photos.length} photo${photos.length !== 1 ? 's' : ''} saved` : 'Your photo booth memories live here'}
        </p>
        <Link href="/photobooth" style={{
      ...
      {!loading && photos.length === 0 && !error && (
        <div style={{ textAlign: 'center', padding: '80px 20px' }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.35)', fontFamily: "'Inter', sans-serif", fontSize: '15px' }}>
            No saved photos yet — go take some! 📷
          </p>
          <Link href="/photobooth" style={{
          ...
          {photos.map((photo, i) => {
            ...
            return (
              <div
                ...
                onClick={() => handleCardClick(photo, i)}
              >
                <img
